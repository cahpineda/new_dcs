...
**Storing user decisions:** Record in `.planning/.checkpoint-state.json` with fields: plan, task, type, response, timestamp. Use `jq` to append to `.checkpoints` array.

**Resuming after checkpoint:** Read last checkpoint from state file, verify has response, continue from next task.

**Handling "none of the above":** Ask for clarification, record as `custom: [description]`. If incompatible with planned tasks, pause for re-planning.

**Checkpoint state cleanup:** After plan completes, archive decisions to SUMMARY.md, then remove from state file.
