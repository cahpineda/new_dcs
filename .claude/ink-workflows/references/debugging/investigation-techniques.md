...
**When to use**: ONLY when a human developer is debugging interactively AND has the ability to revert commits. NOT suitable for AI agent debugging.

**Why AI agents should NOT use this**: Finding the breaking commit doesn't help when you can't revert. The goal is to understand what's wrong in the CURRENT code and fix it forward. Git bisect wastes agent turns on history archaeology instead of root cause analysis.

**For AI agents, prefer instead**: Binary search through the CURRENT code path (see `binary_search` technique above), observability-first logging, or working backwards from the error.
</technique>
