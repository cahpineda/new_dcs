...
If argument provided, use it as AGENT_ID.
Otherwise, read from `.planning/current-agent-id.txt` using the Read tool.
...
Use /ink:progress to check project status.
```
...
Read `.planning/agent-history.json` using the Read tool. If missing, error: no agent history found.
...
```
Resume aborted. Review your changes and run /ink:resume-task when ready.
```
End workflow.
...
Read `.planning/agent-history.json` using the Read tool.
...
Run /ink:go to start fresh if needed.
```
