...
**Optimization:** Run `node bin/ink-tools.js phase add "<description>"` to perform all steps atomically:
creates directory, updates ROADMAP.md section, updates STATE.md total phases, returns phase number and directory path.
If ink-tools.js not available, fall back to manual steps below.

<step name="parse_arguments">
Parse the command arguments:
- All arguments become the phase description
- Example: `/ink:add-phase Add authentication` → description = "Add authentication"
- Example: `/ink:add-phase Fix critical performance issues` → description = "Fix critical performance issues"

If no arguments provided:

```
ERROR: Phase description required
Usage: /ink:add-phase <description>
Example: /ink:add-phase Add authentication system
```
...
2. Insert new phase heading:
   ...
   **Plans:** 0 plans
   ...
   Plans:
   - [ ] TBD (run /ink:plan-phase {N} to break down)
...
**Also available:**
- `/ink:add-phase <description>` — add another phase
- Review roadmap
...
- Don't use decimal numbering (that's /ink:insert-phase)
- Don't create plans yet (that's /ink:plan-phase)
