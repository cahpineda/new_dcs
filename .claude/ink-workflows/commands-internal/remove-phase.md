...
**Primary approach:** Use ink-tools.js for atomic phase removal:

1. Preview changes (dry-run): `node bin/ink-tools.js phase remove <N>`
2. Confirm with user (show preview output)
3. Execute removal: `node bin/ink-tools.js phase remove <N> --confirm`

This replaces the 13-step manual process below with 1 deterministic CLI call.
If ink-tools.js not available, fall back to manual steps below.

<step name="parse_arguments">
Parse the command arguments:
- Argument is the phase number to remove (integer or decimal)
- Example: `/ink:remove-phase 17` → phase = 17
- Example: `/ink:remove-phase 16.1` → phase = 16.1

If no argument provided:

```
ERROR: Phase number required
Usage: /ink:remove-phase <phase-number>
Example: /ink:remove-phase 17
```
...
To abandon current work, use /ink:pause-work instead.
```
...
Update STATE.md fields using ink-tools (do NOT rewrite the full file):

1. **Update total phase count:**
   `node bin/ink-tools.js state set "Phase" "<current> of <new_total>"`

2. **Recalculate progress percentage:**
   `node bin/ink-tools.js state update-progress`

Do NOT add a "Roadmap Evolution" note - the git commit is the record.
Do NOT use the Write tool on STATE.md — use only ink-tools to preserve CurrentTicket/CurrentSessionId.
</step>
...
```
## Ready to Commit Phase Removal

**Files to commit:**
- .planning/ (all modified planning files)
...
2. **Skip commit** - Continue without committing (WARNING: changes already made to filesystem)
...
Would you like to:
- `/ink:progress` — see updated roadmap status
