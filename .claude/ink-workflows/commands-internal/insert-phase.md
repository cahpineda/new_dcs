...
**Optimization:** Run `node bin/ink-tools.js phase insert <after-phase> "<description>"` for atomic decimal phase insertion:
creates directory, calculates next decimal number, updates ROADMAP.md, returns phase ID and directory path.
...
Example: `/ink:insert-phase 72 Fix critical auth bug`
→ after = 72
...
if [ $# -lt 2 ]; then
  ...
  echo "Usage: /ink:insert-phase <after> <description>"
  echo "Example: /ink:insert-phase 72 Fix critical auth bug"
  exit 1
fi
```
...
2. Insert new phase heading with (INSERTED) marker:
   ...
   **Plans:** 0 plans
   ...
   Plans:
   - [ ] TBD (run /ink:plan-phase {decimal_phase} to break down)
...
- Don't use this for planned work at end of milestone (use /ink:add-phase)
