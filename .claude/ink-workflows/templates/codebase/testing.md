...
<good_examples>
Example (condensed Vitest project):

**Framework:** Vitest 1.0.4, vitest.config.ts. Commands: `npm test`, `npm test -- --watch`, `npm run test:coverage`.

**Organization:** `*.test.ts` alongside source files. Structure: `src/lib/parser.ts` + `src/lib/parser.test.ts`.

**Test structure:** `describe('Module') > describe('function') > it('should...')`. Patterns: beforeEach (not beforeAll), vi.restoreAllMocks() in afterEach, arrange/act/assert.

**Mocking:** `vi.mock('./module', () => ({ fn: vi.fn() }))`. Mock: fs, child_process, APIs, env vars. DON'T mock: pure functions, utilities.

**Factories:** `function createTestConfig(overrides?: Partial<Config>): Config { return { ...defaults, ...overrides }; }`. Inline for simple, factory for complex, shared in `tests/fixtures/`.

**Coverage:** No enforced target, tracked for awareness. Vitest c8 built-in.

**Types:** Unit (isolated, mocked, <100ms), Integration (multi-module, external boundaries mocked), E2E (not used).
</good_examples>
